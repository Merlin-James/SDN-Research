...
class Switch (EventMixin):
  ...
  def _handle_flowstats_received (self, event):
    ...
    if self.flow_count >=300 : 
	  ...
	  print dpidToStr(event.connection.dpid),' at ', datetime.now(),'Flows', self.flow_count
          attack = True
  ...
  def _handle_PacketIn (self, event):
    ...
    def drop_pack ():
           cprint('Attack Confirmed : Drop the packets', 'green') 
           msg = of.ofp_packet_out()
           msg.buffer_id = event.ofp.buffer_id
    ...
    if isinstance(packet.next, ipv4):
       ...
       if self.flow_list == 1:
          ...
          #print 'end', self.end
       if self.flow_list > 100 and self.start<self.end:
       ...
       """
       print 'flow', self.flow_list
       if self.flow_list>300:
          if start<end:
             print 'flow count', self.flow_list, '\n'
...
def launch ():
  ...
  Timer(5, Switch.cleaning_sent_sw, recurring = True)
